...
const nodeDrag = drag() // Create a new drag
  .on('start', (d) => { // on start, fix the coordinates
    if (!event.active) simulation.alphaTarget(0.3).restart();
    d.fx = d.x;
    d.fy = d.y;
  })
  .on('drag', (d) => { // update the fixed coordinates on drag
    d.fx = event.x;
    d.fy = event.y;
  })
  .on('end', (d) => { // On end, remove the fixed coordinates
